...
---
## Основные проблемы ИИ с мобильной версткой:
- ИИ часто задаёт фиксированные размеры, которые ломаются на малых экранах;
- Контент не масштабируется под ширину экрана;
- Горизонтальная ориентация - интерфейс «плывёт» из-за сложностей с перерасчётом layout’ов при резком изменении_aspect_ratio;

## Как решал:
- Отключил горизонтальный режим на уровне Info.plist (iOS) и AndroidManifest.xml для принудительной портретной ориентации;
- Тестировал на границах. iPhone SE (375px) как минимальный таргет, Pro Max (428px) как максимальный;
- Ручная проверка ключевых экранов после каждой генерации кода;
- Flutter виджеты с ограничениями: LayoutBuilder, MediaQuery, Flexible/Expanded вместо фиксированных размеров;
